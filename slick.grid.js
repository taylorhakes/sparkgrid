	...
	function SlickGrid(container, data, columns, options) {
		...
		function finishInitialization() {
			if (!initialized) {
				...
				$headerScroller.delegate(".slick-header-column", "mouseenter", handleHeaderMouseEnter);
				$headerScroller.delegate(".slick-header-column", "mouseleave", handleHeaderMouseLeave);
				...
				$canvas.bind("dragend", handleDragEnd)
		...
		function measureScrollbar() {
			...
			var dim = {
				width: $c.style.width - $c.clientWidth,
				height: $c.style.height - $c.clientHeight
			};
...
//			}
		...
		function createColumnHeaders() {
			...
			for (var i = 0; i < columns.length; i++) {
				...
				var header = $("<div class='ui-state-default slick-header-column' />").html("<span class='slick-column-name'>" + m.name + "</span>").width(m.width - headerColumnWidthDiff).attr("id",
						"" + uid + m.id).attr("title", m.toolTip || "").data("column",
						m).addClass(m.headerCssClass || "").appendTo($headers);
				...
				if (options.showHeaderRow) {
					var headerRowCell = $("<div class='ui-state-default slick-headerrow-column l" + i + " r" + i + "'></div>").data("column",
						m).appendTo($headerRow);
		...
		function getViewportHeight() {
			return parseFloat($.css($container[0], "height", true)) - parseFloat($.css($container[0], "paddingTop",
				true)) - parseFloat($.css($container[0], "paddingBottom", true)) - parseFloat($.css($headerScroller[0],
				"height")) - getVBoxDelta($headerScroller) - (options.showTopPanel ? options.topPanelHeight + getVBoxDelta($topPanelScroller) : 0) - (options.showHeaderRow ? options.headerRowHeight + getVBoxDelta($headerRowScroller) : 0);
		}
		...
		function renderRows(range) {
			var parentNode = $canvas[0], stringArray = [], rows = [
