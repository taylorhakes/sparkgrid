	...
	function HeaderMenu(options) {
		...
		function hideMenu() {
			if ($menu) {
				$menu.remove();
				$menu = null;
				$activeHeaderColumn.removeClass("spark-header-column-active");
			}
		}
		...
		function handleHeaderCellRendered(e, args) {
			...
			if (menu) {
				var $el = $("<div></div>").addClass("spark-header-menubutton").data("column", column).data("menu", menu);
		...
		function showMenu(e) {
			...
			if (!$menu) {
				$menu = $("<div class='spark-header-menu'></div>").appendTo(_grid.getContainerNode());
			}
			$menu.empty();
			...
			for (var i = 0; i < menu.items.length; i++) {
				...
				var $li = $("<div class='spark-header-menuitem'></div>").data("command", item.command || "").data("column", columnDef).data("item", item).bind("click", handleMenuItemClick).appendTo($menu);
				...
				var $icon = $("<div class='spark-header-menuicon'></div>").appendTo($li);
				...
				$("<span class='spark-header-menucontent'></span>").text(item.title).appendTo($li);
			}
			...
			$activeHeaderColumn.addClass("spark-header-column-active");
