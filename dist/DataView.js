(function (factory) {
	if (typeof define === "function" && define.amd) {
		define(["exports", "module", "./core", "./GroupItemMetadataProvider"], factory);
	} else if (typeof exports !== "undefined" && typeof module !== "undefined") {
		factory(exports, module, require("./core"), require("./GroupItemMetadataProvider"));
	}
})(function (exports, module, _core, _GroupItemMetadataProvider) {
	"use strict";
  ...
  */
	...
	var GroupTotals = _core.GroupTotals;
	var EventControl = _core.EventControl;
	var GroupItemMetadataProvider = _interopRequire(_GroupItemMetadataProvider);
   ...
   */
		...
		function compileFilter() {
			...
			var filterBody = filterInfo.body.replace(/return false\s*([;}]|$)/gi, "{ continue _coreloop; }$1").replace(/return true\s*([;}]|$)/gi, "{ _retval[_idx++] = $item$; continue _coreloop; }$1").replace(/return ([^;}]+?)\s*([;}]|$)/gi, "{ if ($1) { _retval[_idx++] = $item$; }; continue _coreloop; }$2");
		...
		function compileFilterWithCaching() {
			...
			var filterBody = filterInfo.body.replace(/return false\s*([;}]|$)/gi, "{ continue _coreloop; }$1").replace(/return true\s*([;}]|$)/gi, "{ _cache[_i] = true;_retval[_idx++] = $item$; continue _coreloop; }$1").replace(/return ([^;}]+?)\s*([;}]|$)/gi, "{ if ((_cache[_i] = $1)) { _retval[_idx++] = $item$; }; continue _coreloop; }$2");
		...
		function getRowDiffs(rows, newRows) {
			...
			for (var i = from, rl = rows.length; i < to; i++) {
				if (i >= rl) {
					...
					if (groupingInfos.length && (eitherIsNonData = item.__nonDataRow || r.__nonDataRow) && item.__group !== r.__group || item.__group && !item.equals(r) || eitherIsNonData && (item.__groupTotals || r.__groupTotals) || item[idProperty] != r[idProperty] || updated && updated[item[idProperty]]) {
						diff[diff.length] = i;
					}
				}
			}
